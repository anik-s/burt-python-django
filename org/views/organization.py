...
from org.models import Organization, City, OrganizationHasCategory, OrgSubOrganization, OrganizationHasBusinessModelType
...
class OrganizationCreateView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, 'dashboard/organization/organization_add.html', context)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form_valid:
            ...
            pbm_types = form.cleaned_data['pbm_types']
            for pbm_type in pbm_types:
                OrganizationHasBusinessModelType(organization=organization, pbm_type_id=pbm_type).save()
...
class OrganizationUpdateView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        pbm_types = OrganizationHasBusinessModelType.objects.filter(organization=organization)
        existing_pbm_types = []
        for pbm_type in pbm_types:
            existing_pbm_types.append(str(pbm_type.pbm_type.id))
        form = OrganizationForm(initial={'pbm_types': existing_pbm_types, 'industry_categories': existing},
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form_valid:
            ...
            # delete existing record from OrganizationHasBusinessModelType
            OrganizationHasBusinessModelType.objects.filter(organization=organization).delete()
            pbm_types = form.cleaned_data['pbm_types']
            # entry new record to OrganizationHasBusinessModelType
            for pbm_type in pbm_types:
                OrganizationHasBusinessModelType(organization=organization, pbm_type_id=pbm_type).save()
