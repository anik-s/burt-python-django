...
from org.views.helper import get_formatted_date, get_date
...
class EventView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form_valid:
            event = form.save()
            event.start_date = get_date(event.start_date_year, event.start_date_month, event.start_date_day)
            event.end_date = get_date(event.end_date_year, event.end_date_month, event.end_date_day)
            event.save()
            for c in children:
                c.save(event)
