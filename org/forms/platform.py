...
def get_org_pbm_type_choices():
    d = []
    cats = PlatformBusinessModelType.objects.all()
    for c in cats:
        c_name = c.name
        s_cats = PlatformBusinessSubModelType.objects.filter(platform_business_model_type=c)
        s_cat_list = []
        for sc in s_cats:
            s_cat_list.append((str(sc.id), sc.name))
        if len(s_cat_list) > 0:
...
class PlatformForm(forms.ModelForm):
    ...
    markets = forms.CharField(required=False, widget=forms.TextInput(attrs={'class': 'form-control'}))
